...
function generateItems(){
    inputVanStorage.forEach(element => {
        ...
        lijntje.className = "lijntje py-1";      ///!!!!!RIK PLEZ HELP!!!!!!////
        ...
        document.getElementById("aantalbandjesDown" + teller).addEventListener("click", function() {
            ...
            element.prijs =  element.prijs - 69.99;
            element.aantal--;
            divElementAantalCounter.innerHTML = element.aantal;
            divElementTotalePrijsP.innerHTML = `€ ${element.prijs}`;
            prijsPerItem.innerHTML = `€ ${element.prijs}`;
            aantalPerItemP.innerHTML = `${element.aantal} x`;
            totaal = totaal - 69.99;
            totaleaankoopprijs.innerHTML = `€ ${totaal}`;
            sessionStorage.setItem('winkelwagen',JSON.stringify(inputVanStorage));
        });
        ...
        let prijsPerItemContent = document.createTextNode(`€ ${element.prijs}`);
